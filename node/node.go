...
func DefaultNewNode(config *cfg.Config, logger log.Logger) (*Node, error) {
	...
	return NewNode(nil, config, 0,
		privval.LoadOrGenFilePVLean(config.PrivValidatorKeyFile(), config.PrivValidatorStateFile()),
		nodeKey,
...
func doHandshake(
	...
	consensusLogger log.Logger, indexer txindex.TxIndexer) error {

	handshaker := cs.NewHandshaker(stateDB, state, blockStore, genDoc)
	handshaker.SetLogger(consensusLogger)
	handshaker.SetEventBus(eventBus)
	if err := handshaker.Handshake(proxyApp, indexer); err != nil {
...
func createPEXReactorAndAddToSwitch(addrBook pex.AddrBook, config *cfg.Config,
	...
	pexReactor := pex.NewReactor(addrBook,
		&pex.ReactorConfig{
			...
			SeedDisconnectWaitPeriod:     1 * time.Hour,
